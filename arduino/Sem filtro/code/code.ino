...
void loop() {
  ...
  accAngleX = (atan(AccY / sqrt(pow(AccX, 2) + pow(AccZ, 2))) * 180 / PI) + 2.69;
  accAngleY = (atan(-1 * AccX / sqrt(pow(AccY, 2) + pow(AccZ, 2))) * 180 / PI) + 0.42;
  ...
  GyroX = GyroX - 0.07; // GyroErrorX ~ (-0.07)
  GyroY = GyroY + 0.38; // GyroErrorY ~ (+0.38)
  GyroZ = GyroZ + 0.09; // GyroErrorZ ~ (+0.09)
  
  // Currently the raw values are in degrees per seconds, deg/s, so we need to multiply by sendonds (s)
